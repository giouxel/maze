package com.giouxel.maze.model;
...
public class Maze {
    ...
    public void initializeMaze(String text) {
    ...
    public void printPath(List<Coordinate> path) {
        ...
        for (Coordinate coordinate : path) {
            if (isStart(coordinate.getX(), coordinate.getY()) || isGoal(coordinate.getX(), coordinate.getY())) {
                continue;
            }
    ...
    public String drawPath(List<Coordinate> path) {
    	StringBuilder resultPath = new StringBuilder();
        for (int i = 0; i < path.size(); i++) {
        	resultPath.append("(" + (path.get(i).getX() + 1) + ":" + (path.get(i).getY() + 1));
        	if (i == 0) {
        		resultPath.append(" (S)), ");
        	}
